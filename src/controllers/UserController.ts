...
export class UserController {
    ...
    app
      ...
      .get("/api/v1/users/stats", this.getUserStats.bind(this), {
        detail: {
          ...
          tags: ["Users"],
          description: "Get system statistics including user count and active connections"
        },
      })
      ...
      .get("/api/v1/users/paginated/:offset/:limit", this.getUsersPaginated.bind(this), {
        detail: {
   ...
   */
  private async updateUser({ params: { id }, body }: { params: { id: string }, body: UpdateUserDto }) {
   ...
   */
  private async getUsersPaginated({ params: { offset, limit } }: { params: { offset: string, limit: string } }) {
    ...
    const result = await this.userService.getUsersPaginated(offsetNum, limitNum);
   ...
   */
  private createSuccessResponse(data: any, message?: string, statusCode: number = 200) {
    const response = {
      success: true,
      data,
      ...(message && { message })
